        ...
        <div className="listagem-container">
            <div className="listagem-header">
                ...
                <div className="listagem-actions">
                    <button className="btn-cadastrar" onClick={() => navigate('/produtos/cadastrar')}>
                        + Novo Produto
                    </button>
            ...
            {loading ? (
                ...
                <div className="produtos-grid">
                    {produtos.map((produto) => (
                        <div key={produto.id} className={`produto-card ${produto.status === 'Inativo' ? 'inativo' : ''}`}>
                            {produto.img && (
                                <img src={produto.img} alt={produto.name} className="produto-img" />
                            )}
                            <div className="produto-info">
                                <h3>{produto.name}</h3>
                                <p>💰 R$ {Number(produto.preco).toFixed(2)}</p>
                                <p>📦 Estoque: {produto.quantidade}</p>
                                <span className={`badge ${produto.status === 'Ativo' ? 'badge-ativo' : 'badge-inativo'}`}>
                                    {produto.status}
                                </span>
                            </div>
                            <div className="produto-botoes">
                                ...
                                {produto.status === 'Ativo' && (
                                    <button className="btn-inativar" onClick={() => handleInativar(produto.id)}>
                                        Inativar
                                    </button>
                                )}
                            </div>
